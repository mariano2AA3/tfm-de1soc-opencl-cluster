...
# ./launcher_blur_filter.sh blur_filter blur.aocx butterflies.ppm width height out.ppm 1 5
...
if [ $# != 7 ]; then
  echo "Uso: ./launcher_blur_filter.sh [program] [kernel] [image] [width] [height] [radius] [sigma]"
  exit -1
fi
...
do
  ...
  ssh root@${NODES[i]} "./launch2.sh $OUTPATH/out $1 -input=imageIn_00_0$i.ppm -w=$WIDTH -h=$HEIGHT -output=out/out_imageIn_00_0$i.ppm -r=$6 -s=$7" &
  echo "done"
done
echo " |- done"
time2=$(get_time)
...
echo "Concating images..."
  pnmcat -leftright out/out*.ppm > out/blur_$6_$7_$3
  rm out/out*.ppm
  rm imageIn_*.ppm
echo " |- done" 
time4=$(get_time)
