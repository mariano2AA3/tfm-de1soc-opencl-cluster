...
bool Image_read( Image* pImage, const char* inputFile )
{
	 ...
	 *-----------------------------------------------------------------------------*/
	if( strstr( inputFile, ".tif" ) != NULL || strstr( inputFile, ".tiff" ) != NULL ) {
...
bool Image_write( Image* pImage, const char* outputFile )
{
	 ...
	 *-----------------------------------------------------------------------------*/
	if( strstr( outputFile, ".tif" ) != NULL || strstr( outputFile, ".tiff" ) != NULL ) {
...
bool Image_scanlineInterleave( Image* pImage )
{
	...
	temp = (uchar*)valloc((pImage->depth/8)*pImage->height*pImage->width*pImage->channels);
...
bool Image_readRaw( Image* pImage, const char* inputFile )
{
	...
	cfgFile = (char*)valloc( sizeof( char ) * cfgLen + 1 );
	...
	pImage->pixels = (uchar*)valloc( sizeof(uchar) * imageBytes );
...
bool Image_readTiff( Image* pImage, const char* inputFile )
{
	...
	pImage->pixels = (uchar*)valloc( size );
	if( pImage->pixels == NULL )
	{
		return false;
	}
